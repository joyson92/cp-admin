  ...
  const [alertMsg, setAlertMsg] = useState({ msg: '', status: 'success' });
    ...
    useEffect(() => {
      const fetchData = async () => {
        try {
          const response = await API.get(`/analytics/invexp`);
                      ...
                      datasets: [
                        {
                          ...
                          backgroundColor: '#789cf2',
                          data: Object.keys(chartData).length > 0 ? chartData['invoice'] : [],
                        },
                        {
                          label: 'Monthly Expense',
                          backgroundColor: '#f39758',
                          data: Object.keys(chartData).length > 0 ? chartData['expense'] : [],
                        },
                      ],
                    }}
                    labels="months"
