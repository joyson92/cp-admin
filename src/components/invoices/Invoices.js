...
import { cilPen, cilSortAscending, cilSortDescending, cilDelete, cilSearch } from '@coreui/icons';
import {
  CCol,
  CRow,
  CCard,
...
const Invoices = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await API.get(`/invoices?loc=${branch}&t=${loggedInUser}`);
        if (response.status !== 200) {
          setAlertMsg({msg: 'Failed to fetch invoices. Please contact administrator.', status: 'danger'});
          setShowAlert(true);
        }
                            ...
                            {Array.isArray(currentPageData) && currentPageData.length > 0 ? (
                                          currentPageData.map((item, index) => (
                                            <CTableRow key={index}>
                                             ...
                                             <CTableDataCell>{item.order_dt ? format(item.order_dt, 'dd-MM-yyyy') : null}</CTableDataCell>
                                             <CTableDataCell style={{textAlign: 'right'}}>&#8377;{item.total}</CTableDataCell>
                                             <CTableDataCell style={{textAlign: 'right'}}>&#8377;{item.total - item.payment_amount}</CTableDataCell>
                                             ...
                                             { loggedInUser === '_sa1' ? (
                                                <Link
                                                    ...
                                                    to={{
                                                      ...
                                                      <CIcon icon={cilDelete} className="float-end" title="Update Order" width={16} style={{marginLeft: '10px'}}/>
                                                </Link>
                                                ): null}
                                             {item.order_id === 'B' ? (
                                                <Link
