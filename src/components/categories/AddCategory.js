...
const AddCategory = () => {
  ...
  const handleSubmit = async(event) => {
      ...
      if (form.checkValidity() === false) {
         ...
         setValidated(true)
          ...
          try {
            const response = await API.post('/categories/add', formObj);
  ...
  <> {loading ? ( <Loader /> ): ''} </>
    ...
    <CForm
        ...
        >
          <CCol md={4}>
            <CFormLabel htmlFor="categoryNameId">Category Name:</CFormLabel>
            <CFormInput type="text" name="category_name" id="categoryNameId" required/>
            <CFormFeedback invalid>Please enter category name.</CFormFeedback>
          </CCol>
